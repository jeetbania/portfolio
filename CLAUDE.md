...
- `src/lib/theme.tsx` — `ThemeProvider` + `useTheme()` hook. First visit follows the OS's `prefers-color-scheme` (and keeps tracking it live if the OS theme changes mid-session); an explicit manual toggle persists to localStorage and wins from then on, overriding the OS setting on every later visit. (Reversed from an earlier "always light by default" decision — Aug 2026.)
...
4. **Case study images**: `next.config.ts` sets `images.qualities` (85 for case-study photos, see AnchoredImage) and `images.minimumCacheTTL` (1 day — see next.config.ts's comment for why not longer), but otherwise Next's built-in Image Optimization is stock: on-the-fly resize + AVIF/WebP re-encode per requested size, served through Vercel's Image Optimization API in production, cached after first request. That optimizes what a *visitor's browser* downloads — it does nothing for the source file sitting in `public/` or in git, and `next dev` doesn't run it at all. Compress/resize source images before adding them to `public/` rather than committing multi-MB files as-is (repo bloat, slower clones/dev). `next.config.ts` also has a `headers()` rule giving raw static files in `public/` (photos next/image doesn't touch, hero videos) that same 1-day `Cache-Control` — without it, Vercel's own default is `max-age=0, must-revalidate` (a network round-trip on literally every view, confirmed via curl against production, Aug 2026).
