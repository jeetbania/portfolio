...
4. **Case study images**: `next.config.ts` currently has `images: { unoptimized: true }` for local dev convenience — revisit before a real production deploy if image optimization matters.
